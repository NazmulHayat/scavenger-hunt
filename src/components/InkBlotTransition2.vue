...
class AnimLoop {
    constructor(fps, el) {
        ...
        this.loop=()=>{
            ...
            var scale=Math.max(window.innerWidth/images[this.frame].width,
            ...
            this.ctx.drawImage(images[this.frame], x, y, images[this.frame].width*scale, images[this.frame].height*scale);
            ...
            if(TimeCompleted>=this.Interval) {
                ...
                if(this.frame<=-1 || this.frame>=frames)
                {
                    ...
                    this.ctx.clearRect(0, 0, this.el.width, this.el.height);
                    document.getElementById("transition").dispatchEvent(new Event("transitionend"));
                    return;
                }
            }
...
export default {
    ...
    mounted(){
        document.getElementById("tran-bg")
            .style.backgroundImage="url(" + images[frames-1].src +  ")";
